...
void W25Q_BlockErase(uint16_t block); //擦除0~1023块  Bloc = Page/64
void W25Q_ChipErase(void); //擦除整个芯片
void W25Q_DataStorage(uint32_t addr,uint8_t *buffer); //向Page写入数据
void W25Q_WriteInstruction(uint8_t instuction); //向寄存器写入数据
uint8_t W25Q_ReadData(uint8_t addr);//从寄存器读取数据
void  W25Q_DataReceive(uint8_t *buffer,uint16_t page); //从Page中读取数据
